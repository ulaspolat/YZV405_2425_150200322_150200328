...
# Path setup
DATASET_DIR = Path("dataset/it")
DATASET_DIR.mkdir(parents=True, exist_ok=True)
OUTPUT_FILE = DATASET_DIR / "idioms_data.json"
IDIOMS_FILE = Path("idioms/it-idioms.json")

# Load Italian idioms from JSON file
def load_idioms(file_path):
    ...
    for idiom, meanings in idioms_data.items():
        idioms_list.append({
            ...
            "significato fig": meanings.get("significato_fig", ""),
            "significato letterale": meanings.get("significato_letterale", "")
        })
    
    return idioms_list

def create_prompt_idiomatic(idiom_data, num_examples=EXAMPLES_PER_REQUEST_IDIOMATIC):
    return f"""Sei un esperto di lingua italiana e dovresti usare la nostra lingua in modi ricchi e creativi.
    Per favore evita modelli come la ripetizione di strutture simili e l'uso di nomi di persone nelle tue frasi.

Compito: creare {num_examples} frasi di esempio in cui la seguente espressione idiomatica viene utilizzata nel suo significato figurato (idiomatico):

Espressione: {idiom_data['espressione idiomatica']}
Significato figurato: {idiom_data['significato fig']}

Restituisci le seguenti informazioni per ogni frase in formato JSON:
1. "sentence": La frase che hai creato
2. "tokenized_sentence": La frase divisa in parole e segni di punteggiatura (in formato array)
3. "expression": L'espressione utilizzata (es: "{idiom_data['espressione idiomatica']}")
4. "category": "idiomatic"
5. "indices": Gli indici (a partire da zero) di tutte le parole dell'espressione come appare nella frase (con flessioni o affissi) all'interno di tokenized_sentence (ad esempio: [3, 4, 5]). Deve includere singolarmente gli indici di tutte le parole che compongono l'espressione.

Regole:
- Usa tutte le parole dell'espressione nella frase
- Preferisci diverse strutture di frase (affermativa, negativa, interrogativa, imperativa, ecc.)
- Usa in diversi contesti (formale, informale, umoristico, accademico, ecc.)
- Usa le espressioni con le loro naturali forme flesse
- Genera almeno {num_examples} frasi diverse
- Assicurati che la frase sia significativa e corretta

Rispondi SOLO nel seguente formato JSON. Non aggiungere spiegazioni o testi aggiuntivi:

```json
[
  {{
    "sentence": "Qui verrà scritta una frase contenente l'espressione.",
    "tokenized_sentence": ["Qui", "verrà", "scritta", "una", "frase", "contenente", "l'", "espressione", "."],
    "expression": "{idiom_data['espressione idiomatica']}",
    "category": "idiomatic",
    "indices": [1, 2]
  }},
  ...
...
IMPORTANTE: Assicurati che la tua risposta sia analizzabile come JSON. Usa solo il formato JSON mostrato sopra. Non aggiungere spiegazioni o informazioni aggiuntive."""

def create_prompt_literal(idiom_data, num_examples=EXAMPLES_PER_REQUEST_LITERAL):
    return f"""Sei un esperto di lingua italiana e dovresti usare la nostra lingua in modi ricchi e creativi.
    Per favore evita modelli come la ripetizione di strutture simili e l'uso di nomi di persone nelle tue frasi.

Compito: creare {num_examples} frasi di esempio in cui la seguente espressione viene utilizzata nel suo significato letterale:

Espressione: {idiom_data['espressione idiomatica']}
Significato letterale: {idiom_data['significato letterale']}
...
def generate_examples(idiom_data, category, total_examples):
    ...
    for i in range(num_requests):
        ...
        print(f"  Generating {category} examples batch {i+1}/{num_requests} ({current_request_size} examples) for '{idiom_data['espressione idiomatica']}'")
        ...
        while retry_count < max_retries and len(examples) < current_request_size:
            ...
            if response:
                ...
                if retry_count < max_retries:
                    ...
                    print(f"    API call failed after {max_retries} attempts for '{idiom_data['espressione idiomatica']}'.")
...
def main():
    ...
    it_idioms = load_idioms(IDIOMS_FILE)
    print(f"Loaded {len(it_idioms)} Italian idioms from {IDIOMS_FILE}")
        
    print(f"Starting generation for {len(it_idioms)} Italian idioms using GPT-4.1-mini...")
    ...
    for i in range(0, len(it_idioms), BATCH_SIZE):
        batch = it_idioms[i:i+BATCH_SIZE]
        print(f"\nStarting batch {i//BATCH_SIZE + 1}/{(len(it_idioms) + BATCH_SIZE - 1) // BATCH_SIZE}")
        
        for idx, idiom_data in enumerate(batch):
            print(f"\nProcessing idiom {i+idx+1}/{len(it_idioms)}: {idiom_data['espressione idiomatica']}")
            
            all_examples = []
            
            # Generate idiomatic examples in batches
            print(f"Generating {IDIOMATIC_EXAMPLES_PER_IDIOM} idiomatic examples for '{idiom_data['espressione idiomatica']}'")
            ...
            print(f"Generating {LITERAL_EXAMPLES_PER_IDIOM} literal examples for '{idiom_data['espressione idiomatica']}'")
            ...
            if all_examples:
                ...
                print(f"Added total {len(all_examples)} examples for '{idiom_data['espressione idiomatica']}' ({sum(1 for e in all_examples if e['category'].lower() == 'idiomatic')} idiomatic, {sum(1 for e in all_examples if e['category'].lower() == 'literal')} literal)")
            else:
                print(f"No valid examples generated for '{idiom_data['espressione idiomatica']}'.")
